...
export const EditorHeader = ({ title, onTitleChange, onBack, canEdit, isLive, saveStatus, activeUsers, currentUser }: EditorHeaderProps) => {
  const getInitials = (email: string) => email.substring(0, 2).toUpperCase();
  
  // Extract just the name part of the email (e.g. "john" from "john@doe.com")
  const getName = (email: string) => email.split('@')[0];

  // Find who is typing (excluding ourselves)
  const typingUsers = activeUsers.filter(u => u.isTyping && u.userId !== currentUser?.id);

  // Sort so the Current User is always the first avatar on the left
  ...
  return (
    <header className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100 gap-4">
        ...
        {typingUsers.length > 0 && (
          <span className="text-blue-500 text-xs italic animate-pulse hidden sm:block">
            {typingUsers.length === 1 
              ? `${getName(typingUsers[0].email)} is typing...` 
              : 'Multiple people typing...'}
          </span>
        )}

        {/* AVATAR STACK WITH CUSTOM TOOLTIPS */}
            ...
            {sortedUsers.map((activeUser) => {
              ...
              return (
                <div key={activeUser.userId} className="relative group">
                  {/* The Avatar Circle */}
                  <div 
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white shadow-sm transition-transform group-hover:z-10 group-hover:scale-110 cursor-help relative
                  ...
                  {/* The Custom CSS Tooltip (Instant Hover) */}
